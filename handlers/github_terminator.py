...
class Handler(BaseHandler):
    ...
    def process(self, channel, user, ts, message, at_bot, extra=None):
        if at_bot:
            if message.startswith('desligar ') or message.startswith('terminate '):
                ...
                tpl = self.bot.get_config(self.config_section, 'nomfa')
                ...
                self.post_message(channel, '@{} usuários adicionados à lista de exclusões de MFA: {}'.format(' '.join(to_allow)))

            elif message.startswith('github deny nomfa '):
                to_deny = message.replace('github allow nomfa ', '').split()

                tpl = [x for x in self.bot.get_config(self.config_section, 'nomfa').split() if x not in to_deny]

                self.bot.write_config(self.config_section, 'nomfa', ' '.join(tpl))

                self.post_message(channel, '@{} usuários removidos da lista de exclusões de MFA: {}'.format(' '.join(to_deny)))
            elif message.startswith('github list nomfa'):
                self.post_message(channel, '@{} MFA Exclusion: {}'.format(self.mfaexclusion))
